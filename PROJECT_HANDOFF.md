...
- 修复 AI 把 `[填空题][单选题]` 写入题干导致题型误判：App 会清理题干标签，并根据选项/答案重新归一化题型。
- 优化 AI 解析提示：解析必须说明为什么答案成立，不能只复述“正确选项是 X”。
- 优化题号导航：搜索改为手动触发，避免输入时反复重绘导致卡顿。
- 优化键盘体验：编辑/导入文本框按键盘换行会收起键盘。

## 已知风险和后续建议

- 文档解析仍可能受 PDF 排版、扫描质量、题目编号混乱影响。AI 辅助会检查题型、选项、答案和解析，但不能保证 100%，必须通过校验预览确认后再应用。
